...
import cl.gob.ips.pagos_asignacion_familiar.dto.ResultadoComunaDTO;
...
@Override
public ListaComuna validarComuna(int codigoRegion, String comuna) {
    Integer idCoincidencia = 0;
    ListaComuna comunaEncontrada = new ListaComuna();
    //Se busca la comuna en la tabla mantenedor de comuna
    comunaEncontrada = comunaDAO.obtenerIdComunaPorNombre(comuna);

    if(comunaEncontrada==null){ //Si no se consigue la comuna en tabla mantendor, se debe validar que exista en Junar
        ...
        //if (regionDTO != null && regionDTO.getResultado() != null) {
                ...
                if (comunaDTO != null && comunaDTO.getResultado() != null) {
                    for (ListaComuna comunaExistente : comunaDTO.getResultado().getListaComunas()) {
